...
def set_all_dim():
    base_color = (160, 170, 100)  # White color for the dim state
...
def play_effect(duration_minutes):
    ...
    while True:
        ...
        if time.time() - lightning_timer >= 10:  # Trigger lightning every 10 seconds
            storm_effect()
            lightning_timer = time.time()  # Reset lightning timer

        # Randomly choose LEDs to brighten as "raindrops"
        if random.random() < 0.42:  # Lower chance for new raindrops
        ...
        time.sleep(0.007)  # Short delay for smoother transitions
